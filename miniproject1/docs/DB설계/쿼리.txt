...
WHERE BOARD_NO = 1;
-- insert
INSERT INTO TB_BOARD (user_no, board_title, board_content,BOARD_PASS) VALUES (2, '게시물 테스트', '게시물 테스트',1004);
-- update
...
-- update
SELECT * FROM TB_USER;
UPDATE TB_USER SET user_pass = '1234', user_name = 'test', user_phone = '010-5678-1234', user_addr = '서울특별시 강서구 화곡본동' WHERE user_no = 1;
...
-- 이름과 전화번호로 아이디 찾기
SELECT USER_ID FROM TB_USER WHERE USER_NAME ='정혜림' AND USER_PHONE='010-6349-8285' AND USER_DELETE_YN='N';
...
-- 아이디, 이름, 전화번호로 비밀 번호 변경
UPDATE TB_USER 
SET USER_PASS = '1234'
WHERE USER_ID = 'hyerim' AND USER_NAME = '정혜림' AND USER_PHONE = '010-6349-8285';
...
WHERE H.user_no = 1;
					...
					UPDATE TB_USER SET USER_DELETE_YN ='N' WHERE USER_NO = 1;
COMMIT;
SELECT * FROM TB_USER 
;


ALTER TABLE TB_USER MODIFY (user_name VARCHAR2(20));
